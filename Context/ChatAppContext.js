...
export const ChatAppProvider = ({ children }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      //console.log(account);
      if(account === "") {
        console.log("Da toi ktra account");
      ...
      console.log("Kiem tra",account,isUserLoggedIn);
      ...
      alert("chung ta cua tuong lai",error);
  ...
  const createAccount = async ({name,userAddress,password,mnemonic}) => {
    ...
    try {
      ...
      console.log(name,userAddress,password,mnemonic);
      const contract = await connectingWithContract();
  ...
  const changePassword = async ({userAddress, cpassword, oldPassword }) => {
    try {
      console.log("vao duoc day chua1",userAddress);
      const contract = await connectingWithContract();
      console.log("vao duoc day chua2",contract);
      console.log("Calling verifyPassword with:", oldPassword);
      const isCorrectPassword = await contract.verifyPassword(oldPassword);
      console.log("verifyPassword result:", isCorrectPassword);
      if(isCorrectPassword){
        console.log("vao duoc day chua");
        console.log("isUserLoggedIn",isUserLoggedIn);
        console.log("myAddress",userAddress);
        console.log("account",account);
        if(isUserLoggedIn && userAddress.toLowerCase() === account)
        {
          console.log("vao duoc day chua sieu nhan");
          ...
          console.log("Da toi cho goi smart contract");
          setLoading(true);
          const rc = await change.wait();
          const event = rc.events.find((event) => event.event === "ChangePasswordUser");
          const [state] = await event.args;
          console.log("ChangePasswordUser",state);
          if (state) {
            setLoading(false); 
            const userMnemonic = localStorage.getItem(account);
            const unscramble = await unscrambleString(scrambledData);
            const mnemonicDecrypted = await decryptMnemonic(userMnemonic,unscramble,account);
            console.log("Mnemonic",mnemonicDecrypted);
            sessionStorage.removeItem(account);
